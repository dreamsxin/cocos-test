...
export default class MoveState extends FSMState {
    ...
    public Reason(npc: cc.Node): void {
        if (Global.Instance(Global).attackEnemyTangseng) {
            Global.Instance(Global).attackEnemyTangseng = false;
            this.fsm.performTransition(Transition.attackEnemy);
        }
        if (Global.Instance(Global).hitEnemyTangseng) {
            Global.Instance(Global).hitEnemyTangseng = false;
